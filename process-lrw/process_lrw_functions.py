...
def process_lrw(dataDir=LRW_DATA_DIR,
    ...
    # For each word
    for setDir in tqdm.tqdm(sorted(glob.glob(os.path.join(dataDir, '*/')))):
        setName = setDir.split('/')[-2]
        if not(setName=="trainval"):
            continue

        print_time_till_now(start_time)
        # Create directory in saveDir if it doesn't exist
        setSaveDir = os.path.join(saveDir, setName)
        ...
        for urlDir in tqdm.tqdm(sorted(glob.glob(os.path.join(setDir, '*/')))):
            ...
            # Create directory in setDir if it doesn't exist
            urlSaveDir = os.path.join(saveDir, setName, url)
            if not os.path.isdir(urlSaveDir):
                os.makedirs(urlSaveDir)

            # Read all .txt file names (since .txt are saved in both
            # LRW_DATA_DIR and LRW_SAVE_DIR)
            # For each video
            for idFilename in tqdm.tqdm(sorted(glob.glob(os.path.join(urlDir, '*.txt')))):
                ...
                if copyTxtFile:
                    try:
                        copy_txt_file(saveDir, idFilename, verbose)
                    except ValueError as err:
                        print(err)
                    except KeyboardInterrupt:
                        print("\n\nCtrl+C was pressed!\n\n")
                ...
                if extractAudioFromMp4:
                    try:
                        extract_audio_from_mp4(saveDir=saveDir, wordFileName=idFilename,
                ...
                def please_extract():
                    ...
                    # Memory I/O error
                        ...
                        print("Trying again...")
                        return please_extract()
                    
                    # File non-existence error
                    except ValueError as err:
                        print("please_extract:", err)
                        return 1
...
def copy_txt_file(saveDir, textFile, verbose=False):
    # Names
    fromFileName = textFile
    toFileName = os.path.join(saveDir, "/".join(textFile.split("/")[-3:]))
#     try:
...
def extract_and_save_frames_and_mouths(saveDir=LRW_SAVE_DIR,
    ...
    try:
        ...
        for f, frame in enumerate(videoFrames):
            ...
            if detectAndSaveMouths:
                face, mouthCoords = detect_mouth_and_write(saveDir=saveDir,
                                ...
                                verbose=verbose)
                features_dict[frameNumer]={"mouthCoords": mouthCoords.tolist()}
    except imageio.core.format.CannotReadFrameError as err:
        print(err)
                        
    feeatures_filename = os.path.join(saveDir, "/".join(wordFileName.split(
                              "/")[-3:]).split('.')[0] + \
                              ".json")
    print("======================"+feeatures_filename+"=============================")
    with open(feeatures_filename, 'w') as outputfile:
        json.dump(features_dict, outputfile)
...
def detect_mouth_and_write(saveDir, wordFileName, frameNumer, frame, detector, predictor,
    ...
    mouthImage, face, mouthCoords = detect_mouth_in_frame(frame, detector, predictor,
...
#     print(face.width(), face.height())
    ...
    expandedMouthRect = expand_rect(mouthRect,
        scale=(MOUTH_TO_FACE_RATIO * max(face.width(), 100) / mouthRect[2]),
    ...
    # Return mouth
    return resizedMouthImage, face, mouthCoords
